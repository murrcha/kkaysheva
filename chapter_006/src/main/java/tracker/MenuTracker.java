...
class EditItem extends BaseAction {
    ...
    public void execute(Input input, Tracker tracker) {
        ...
        Item existingItem = tracker.findById(Integer.valueOf(id.trim()));
        if (existingItem != null) {
            String name = input.ask("Введите новое имя заявки: ");
            String description = input.ask("Введите новое описание заявки: ");
            Item newItem = new Item(name, description);
            tracker.replace(existingItem.getId(), newItem);
            System.out.println(String.format("Изменена заявка ID: %s| Имя: %s| Описание: %s", existingItem.getId(), newItem.getName(), newItem.getDescription()));
        } else {
